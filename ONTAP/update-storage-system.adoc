...
---
= Inicie a atualização do ONTAP em atualizações de software
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Para obter informações sobre como identificar a versão de destino do ONTAP, consulte link:../ONTAP/choose-ontap-910-later.html["Compare as versões do ONTAP com as atualizações de software"].

. Corrija e reconheça os bloqueadores do ONTAP.
+
Para obter informações sobre como corrigir bloqueadores, consulte link:../ONTAP/fix-blockers-warnings.html["Corrija os bloqueios e avisos do ONTAP usando atualizações de software"].
